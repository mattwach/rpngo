...
func (wc *WindowManagerCommands) wListP(r *rpn.RPN) error {
	...
	w := wc.manager.FindWindow(wname.UnsafeString())
	if w == nil {
...
func (wc *WindowManagerCommands) wGetP(r *rpn.RPN) error {
...
func (wc *WindowManagerCommands) wSetP(r *rpn.RPN) error {
	f, err := r.PopFrame()
	if err != nil {
		return err
	}
...
func (wc *WindowManagerCommands) wSnapshot(r *rpn.RPN) error {
	wname, err := r.PopFrame()
	if err != nil {
		return err
	}
	...
	buff, err = wc.manager.Snapshot(buff, wname.UnsafeString())
	if err != nil {
		return err
	}
...
func (wc *WindowManagerCommands) snapshot(r *rpn.RPN) error {
	buff := make([]byte, 0, 256)
	buff = append(buff, []byte("d\nv.clearall\nw.reset\n")...)
	buff, _ = wc.manager.Snapshot(buff, "root")
